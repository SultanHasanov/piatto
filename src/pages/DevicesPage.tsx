...
export const DevicesPage = observer(function DevicesPage() {
  ...
  const [pairName,setPairName]=useState(''), [nameOpen,setNameOpen]=useState(false)
  ...
  async function create(){if(!auth.deviceId||!pairName.trim())return;setBusy(true);try{const pair=await api.createDevicePairing(auth.deviceId,pin,pairName.trim());const webBase=location.protocol==='http:'||location.protocol==='https:'?`${location.origin}${location.pathname}`:'https://piatto-three.vercel.app/';const link=`${webBase}?pair=${pair.token}`;setQr(await QRCode.toDataURL(link,{width:280,margin:2}));setCode(pair.code);setNameOpen(false)}catch{message.error('Не удалось создать код сопряжения')}finally{setBusy(false)}}
  ...
  return <div className="page-container">
    ...
    {!authorized ? <Card style={{maxWidth:440}}>
      ...
      <TerminalNumericInput mode="pin" maxLength={4} masked value={pin} onChange={value=>setPin(String(value??''))} alwaysOpen/>
      <Button type="primary" block loading={busy} disabled={pin.length!==4} onClick={load}>Открыть</Button>
    </Card> : <>
      <Space wrap>
        <Button type="primary" loading={busy} onClick={()=>{setPairName('');setNameOpen(true)}}>Привязать новое устройство</Button>
    ...
    <Modal title="Новое устройство" open={nameOpen} onCancel={()=>setNameOpen(false)} onOk={create} okText="Создать код" confirmLoading={busy} okButtonProps={{disabled:!pairName.trim()}} centered>
      <Typography.Paragraph type="secondary">Название задаёт администратор и пользователь нового устройства не сможет его изменить.</Typography.Paragraph>
      <Input autoFocus value={pairName} maxLength={80} placeholder="Например, Касса у бара" onChange={event=>setPairName(event.target.value)} onPressEnter={()=>{if(pairName.trim())void create()}}/>
    </Modal>
    <Modal open={!!qr} footer={null} onCancel={()=>setQr(undefined)} title={`Сопряжение: ${pairName}`}><div style={{textAlign:'center'}}><img src={qr} alt="QR-код сопряжения" style={{maxWidth:'100%'}}/><Typography.Title level={3}>{code}</Typography.Title><Typography.Paragraph type="secondary">Отсканируйте QR или введите код на новой кассе. Код действует 10 минут и только один раз.</Typography.Paragraph></div></Modal>
  </div>
})
