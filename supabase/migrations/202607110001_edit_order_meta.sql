-- Разрешаем менять способ оплаты и тип заказа при редактировании заказа (editOrder).
-- Ранее ветка editOrder обновляла только total и data, из-за чего денормализованные
-- колонки payment_method / order_type / order_type_name / order_type_surcharge
-- оставались старыми. Пересоздаём функцию с обновлением этих колонок.
